  ...
  return (
    <div
      className={`rounded-3xl border transition-all duration-300 flex flex-col justify-between overflow-hidden shadow-2xl relative h-full ${
        isPositive
          ? "bg-slate-900/60 border-slate-800/90 shadow-emerald-950/20"
          : "bg-slate-900/60 border-slate-800/90 shadow-rose-950/20"
      }`}
      style={{
    ...
    >
      ...
      <div className="px-5 py-4 bg-slate-950/85 border-b border-slate-800/80 flex items-center justify-between gap-3 shrink-0">
      ...
      <div className="p-4 sm:p-5 lg:p-6 flex-1 flex flex-col justify-between space-y-3.5 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="space-y-4">
          ...
          <motion.div
            ...
            className="rounded-2xl p-4 border bg-slate-950/70 border-slate-800/90 relative"
          >
            ...
            <p className="text-sm lg:text-[15px] font-semibold text-slate-100 leading-relaxed">
              {scenario.question}
            </p>
          </motion.div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-bold text-slate-200">You (Student)</span>
                <span className="text-[10px] text-slate-400">Technical Round</span>
              </div>
              <div
                ...
                {scenario.companionResponse.map((bubble, i) => (
                  ...
                  >
                    <TypewriterBubble
                      ...
                      style={{
                        backgroundColor: `${mentor.signatureColor}12`,
                        borderColor: `${mentor.signatureColor}30`,
                        color: "#f8fafc",
                      }}
                    />
                  </motion.div>
        ...
        <motion.div
          ...
          className="mt-4 pt-3 border-t border-slate-800/80 shrink-0"
        >
          <div
            className={`p-3.5 rounded-2xl border-l-4 border bg-slate-950/60 text-xs sm:text-[13px] leading-relaxed ${
              isPositive
                ? "border-l-emerald-400 border-slate-800/70 text-slate-300"
                : "border-l-rose-400 border-slate-800/70 text-slate-300"
            }`}
          >
            ...
            <p className="text-slate-300 font-normal">{scenario.takeaway}</p>
          </div>
        </motion.div>
      </div>
    </div>
...
const CompactCoachCard = memo(function CompactCoachCard({
  ...
  return (
    <motion.button
      ...
      style={{
        borderColor: isSelected ? mentor.signatureColor : undefined,
        boxShadow: isSelected
          ? `0 0 28px ${mentor.glowColor}, 0 10px 24px rgba(0,0,0,0.6)`
          : undefined,
      }}
    >
      ...
      <div className="relative shrink-0">
        <div
          ...
          style={{
            backgroundColor: isSelected ? `${mentor.signatureColor}20` : "rgba(15,23,42,0.6)",
            borderColor: isSelected ? `${mentor.signatureColor}60` : "rgba(51,65,85,0.4)",
          }}
        >
...
export function StepCompanion({
  ...
  const { setCompanion, setPreviewCompanion } = useCompanionTheme();
  ...
  return (
    <div
      ...
      tabIndex={0}
      className="w-full h-full flex flex-col lg:flex-row relative outline-none overflow-hidden bg-slate-950"
    >
      {/* =================================================================== */}
      ...
      <div className="flex-1 h-full p-3 sm:p-5 lg:p-6 flex flex-col justify-between overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden relative z-10">
        ...
        <div className="flex items-center justify-between gap-4 pb-4 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-slate-300 text-xs font-medium hidden sm:inline truncate max-w-md">
              &ldquo;{activeMentor.catchphrase}&rdquo;
            </span>
          </div>

          <div className="flex items-center gap-2.5">
            <div className="hidden md:flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-950/80 border border-slate-800 text-xs shadow-sm">
              <Palette className="h-3.5 w-3.5" style={{ color: activeMentor.signatureColor }} />
              <span className="text-slate-400">Recommended Theme:</span>
        ...
        */}
        ...
        <div className="pt-4 flex lg:hidden items-center justify-between border-t border-slate-900 shrink-0 mt-3">
          <button
            type="button"
            onClick={onBack}
      ...
      {/* =================================================================== */}
      <div className="w-full lg:w-[440px] xl:w-[480px] 2xl:w-[520px] h-auto lg:h-full shrink-0 flex flex-col justify-between p-4 sm:p-6 bg-slate-900/70 border-t lg:border-t-0 lg:border-l border-slate-800/80 rounded-tl-[36px] backdrop-blur-2xl shadow-2xl relative z-20 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="space-y-4">
          {/* Right Dock Header */}
        ...
        <div className="pt-5 border-t border-slate-800/80 space-y-3 shrink-0">
          <button
            type="button"
            onClick={onNext}
